...
SystemDataConfigRouter.post(
  "/system/data/object/get",
  async (context: Context) => {
    const body = context.request.body({ type: "json" });
    const req = await body.value;

    context.response.body = StoredObjectService.getObject({
      group: req.group,
      object: req.object,
    });
