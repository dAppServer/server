...
export class LetheanWalletCli {
  ...
  static run(args: any) {
    ...
    this.exeFile = "lethean-wallet-cli" +
      (os.platform() === "windows" ? ".exe" : "");
    LetheanWalletCli.command = path.join(
      homeDir ? homeDir : "./",
      "Lethean",
      "cli",
      this.exeFile,
  ...
  public static config() {
    ...
    return new Command()
      ...
      .option(
        "--generate-new-wallet  <string>",
        "Generate new wallet and save it to <arg>",
      )
      .option(
        "--generate-from-view-key  <string>",
        "Generate incoming-only wallet from view key",
      )
      .option(
        "--generate-from-keys  <string>",
        "Generate wallet from private keys",
      )
      .option(
        "--generate-from-multisig-keys  <string>",
      ...
      .option(
        "--upgrade-legacy  <string>",
        "Upgrade a pre-rebase .wallet file <arg> to the new format which is compatible with this wallet",
      )
      .option("--mnemonic-language  <string>", "Language for mnemonic")
      .option(
        "--command  <string>",
        "run command, hint, use exit to stop auto sync",
      )
      .option(
        "--restore-deterministic-wallet  <boolean>",
