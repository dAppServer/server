...
import { Sub , DenoHttpServer} from "https://deno.land/x/jszmq/mod.ts";
...
export class ZeroMQServer {
	...
	public static sendRepMessage(channel: string, message: string){
		console.log(`ZeroMQ ReqRep ${channel} message length: ${message.length}`)
		ZeroMQServer.sockets.reqrep.send([channel, message])
	}

	public static sendPushMessage(channel: string, message: string){
		console.log(`ZeroMQ PushPull ${channel} message length: ${message.length}`)
		ZeroMQServer.sockets.pushpull.send( message)
	}

	private static loadRep() {
		ZeroMQServer.sockets.reqrep = new zmq.Rep()
		ZeroMQServer.sockets.reqrep.bind(ZeroMQServer.socketServer, ZeroMQServer.repEndpoint);
	}

	private static loadPub(){
		ZeroMQServer.sockets.pubsub = new zmq.Pub()
		ZeroMQServer.sockets.pubsub.bind(ZeroMQServer.socketServer, ZeroMQServer.pubEndpoint);
	}

	private static loadPush(){
		ZeroMQServer.sockets.pushpull = new zmq.Push()
		ZeroMQServer.sockets.pushpull.bind(ZeroMQServer.socketServer, ZeroMQServer.pushEndpoint );
	}
