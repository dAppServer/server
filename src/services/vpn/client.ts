...
export class VPNClient {
  public static config() {
    ...
    return new Command()
      ...
      .option(
        "--authid <string>",
        'Authentication ID. Use "random" to generate.',
      )
      .option(
        "--uniqueid <string>",
        'Unique ID of proxy. Use "random" to generate.',
      )
      .option(
        "--stunnel-port <string>",
      ...
      .option(
        "--proxyc-mgmt-port <number>",
        "Use specific port for local Haproxy mgmt",
        { default: 11194 },
      )
      .option(
        "--proxyc-ssl-noverify <boolean>",
        "Do not verify SSL certificate of remote proxy. Dangerous, use only if you know what you are doing!",
      )
      .option(
        "--vpnc-block-route <boolean>",
        "Filter router changes from server",
      )
      .option(
        "--vpnc-block-dns <boolean>",
        "Filter router DNS server from server",
      )
