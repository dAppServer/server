...
import { convertToArray, isPromise } from "../utils/index.ts";
...
export class ValidationExecutor {
  ...
  execute(
    ...
    const always = (this.validatorOptions && this.validatorOptions.always) ||
      false;
    ...
    if (
      this.validatorOptions && this.validatorOptions.forbidUnknownValues &&
  ...
  private performValidations(
    ...
    const customValidationMetadatas = metadatas.filter((metadata) =>
      metadata.type === ValidationTypes.CUSTOM_VALIDATION
    );
    const nestedValidationMetadatas = metadatas.filter((metadata) =>
    ...
    if (
      value === undefined && this.validatorOptions &&
    ...
    if (
      value === null && this.validatorOptions &&
  ...
  private customValidations(
    ...
    metadatas.forEach((metadata) => {
      this.metadataStorage.getTargetValidatorConstraints(metadata.constraintCls)
        .forEach((customConstraintMetadata) => {
          ...
          const validationResult = (validatedSubValues as any).every((
            isValid: boolean,
          ) => isValid);
          if (!validationResult) {
            const [type, message] = this.createValidationError(
  ...
  private nestedValidations(
    ...
    metadatas.forEach((metadata) => {
      ...
      if (
        value instanceof Array || value instanceof Set || value instanceof Map
      ) {
