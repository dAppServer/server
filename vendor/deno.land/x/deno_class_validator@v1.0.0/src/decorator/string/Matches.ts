...
export function matches(
  ...
  return typeof value === "string" &&
    matchesValidator(value, (pattern as unknown) as any, modifiers!);
}
...
export function Matches(
  ...
  if (
    ...
    modifiersOrAnnotationOptions instanceof Object && !validationOptions
  ) {
  ...
  return ValidateBy(
    {
      ...
      validator: {
        validate: (value, args): boolean =>
          matches(value, args!.constraints[0], args!.constraints[1]),
        defaultMessage: buildMessage(
          (eachPrefix, args) =>
            eachPrefix + "$property must match $constraint1 regular expression",
          validationOptions,
        ),
      },
    },
