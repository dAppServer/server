...
function diff(A, B) {
    ...
    var delta = M - N;
    var size = M + N + 1;
    var fp = Array.from({ length: size }, function () { return ({ y: -1, id: -1 }); });
...
function diffstr(A, B) {
    ...
    function unescape(string) {
        return string
            ...
            .replaceAll("\t", "\\t")
            .replaceAll("\v", "\\v")
            .replaceAll(/\r\n|\r|\n/g, function (str) { return str === "\r" ? "\\r" : str === "\n" ? "\\n\n" : "\\r\\n\r\n"; });
    }
    ...
    var diffResult = diff(tokenize("".concat(unescape(A), "\n")), tokenize("".concat(unescape(B), "\n")));
    var added = [], removed = [];
